...
public abstract class Game {
    ...
    public int id, min, teamSize, max, starting, pregame, borderX, borderZ, event = 0;
    public State state;
    ...
    public Game(GameManager gm, String path, String name, int id, CallBackAPI<String> correctly) {
        ...
        if (plugin.getArenas().isSet(path + ".islands")) {
            ...
            for (String c : conf.getKeys(false)) {
                ...
                teams.put(tid, t);
                t.getChest().getInvs().keySet().forEach(e -> chests.put(e, tid));
            }
        } else {
        ...
        center.getInvs().keySet().forEach(l -> chests.put(l, -1));
        ...
        plugin.getWc().deleteWorld(s, (w) -> new BukkitRunnable() {
            ...
            public void run() {
                ...
                teams.values().forEach(t -> {
                    t.reset();
                    t.getChest().getInvs().keySet().forEach(e -> chests.put(e, t.getId()));
                });
                List<Location> chests = new ArrayList<>(center.getInvs().keySet());
                center = new GameChest(true, chests);
                center.getInvs().keySet().forEach(l -> Game.this.chests.put(l, -1));
    ...
    public void addEnderPearl(Player p) {
        UltraSkyWars plugin = UltraSkyWars.get();
        enderpearls.put(p.getUniqueId(), System.currentTimeMillis() + plugin.getCm().getEnderPearlSeconds());
    }
    ...
    public void setSpect(Player p) {
        UltraSkyWars plugin = UltraSkyWars.get();
        NMSReflection.setCollidesWithEntities(p, false);
        if (gamePlayer.containsKey(p.getUniqueId())) {
